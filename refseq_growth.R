...
setwd("~/Dropbox/manuscripts/book_chapter/refseq_growth")
refseq_growth <- read.table("refseq_growth.txt", header=T, sep="\t")
refseq_growth$date <- as.Date(refseq_growth$Date, format="%d-%b-%y")

seq_growth <- read.table("seq_growth.csv", header=T, sep=",")
...
img <- readPNG("kanagawa1_faded.png")
...
quartz(width=16.817460, height=5.212598)
multiplot(p1, p2, p3, cols=3)


seqgrowth_fit <- glm(kb_day ~ date, family = gaussian(link="log"), data=seq_growth)
refseq_fit <- glm(Records ~ Date , family = gaussian(link="log"), data = refseq_growth)
fit_df <- data.frame(sequencing=exp(predict(seqgrowth_fit, as.data.frame(date))), date)

annotation_custom(g, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf)

plot(seq_growth$date, seq_growth$kb_day)
lines(refseq_growth$Date, log(refseq_growth$Records), type = "l")


Date <- as.Date(c("2004","2006","2008","2010","2012","2014"), format="%Y")
date <-  as.Date(c("2004","2006","2008","2010","2012","2014"), format="%Y")
plot(exp(predict(refseq_fit,as.data.frame(Date))))
qplot(exp(predict(seqgrowth_fit, as.data.frame(date))), exp(predict(refseq_fit,as.data.frame(Date)))) + theme_bw(base_family="Gill Sans")


p3 <- ggplot(fit_df, aes(sequencing, refseq)) + 
  theme_bw(base_family="Gill Sans") + 
  geom_segment(aes(xend=sequencing, yend=0), color="blue", size=1.5) + 
  geom_segment(aes(xend=0, yend=refseq), color="red", size=1.5) +
  #scale_x_log10(limits=c(1, max(fit_df$sequencing))) +
  #scale_y_log10(limits=c(1, max(fit_df$refseq))) +
  geom_point(size=4)

# png version using library(png)


img <- readPNG("kanagawa1.png")


ggplot(dat, aes(x = x_data, y = y_data)) + annotation_raster(image, -Inf, Inf, -Inf, Inf, interpolate = TRUE) + geom_point(shape=21, bg = 'red') + geom_line()
